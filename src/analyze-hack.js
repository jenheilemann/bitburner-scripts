...
export async function main(ns) {
  ...
  var server_eval = servers.filter(server => (flags['all'] || server.hasAdminRights && server.requiredHackingSkill <= player.hacking)
    ...
    .map(function (server) {
      ...
      var hackCost = hack_ram * ns.formulas.hacking.hackTime(server, player);
      var weakenCost = weaken_ram * ns.formulas.hacking.weakenTime(server, player);
  ...
  await ns.write('/Temp/analyze-hack.txt', JSON.stringify(server_eval.map(s => ({
    hostname: s.hostname,
    gainRate: s.gainRate,
    expRate: s.expRate
  }))), "w");
