...
const preferredEarlyFactionOrder = [
  ...
  "Daedalus", // Once we have all faction_rep boosting augs, there's no reason not to go for Daedalus as soon as it's available so we can buy Red Pill
...
const argsSchema = [
  ['first', []], // Grind rep with these factions first
...
export async function main(ns) {
  ...
  if (skipFactions.length > 0) ns.print(`--skip factions: ${skipFactions.join(", ")}`);
  ...
  mostExpensiveDesiredAugByFaction = Object.fromEntries(factions.map(f => [f, dictFactionAugs[f]
    .filter(aug => !ownedAugmentations.includes(aug) && (Object.keys(dictAugStats[aug]).length == 0 || !desiredAugStats || Object.keys(dictAugStats[aug]).some(key => desiredAugStats.some(stat => key.includes(stat)))))
    .reduce((max, aug) => Math.max(max, dictAugRepReqs[aug]), -1)]));
  //ns.print("Most expensive desired aug by faction: " + JSON.stringify(mostExpensiveDesiredAugByFaction));
  let completedFactions = Object.keys(mostExpensiveAugByFaction).filter(f => mostExpensiveAugByFaction[f] == -1);
  skipFactions = skipFactions.concat(completedFactions);
  let softCompletedFactions = Object.keys(mostExpensiveDesiredAugByFaction).filter(k => mostExpensiveDesiredAugByFaction[k] == -1 && !completedFactions.includes(k));
  ...
  while (true) { // After each loop, we will repeat all prevous work "strategies" to see if anything new has been unlocked, and add one more "strategy" to the queue
    ...
    if (joinedFactions.length > numJoinedFactions) {
      scope = 0; // Back to basics until we've satisfied all highest-priority work
      numJoinedFactions = joinedFactions.length;
    }
    ...
    let invitedFactions = await getInvitedFactions(ns);
    let allCurrentFactions = invitedFactions.concat(joinedFactions);
    // Skip the removal if we already have the faction invite.
    if (!allCurrentFactions.includes("Fulcrum Secret Technologies")) {
      let fulcrummHackReq = ns.getServerRequiredHackingLevel("fulcrumassets");
      if (ns.getPlayer().hacking < fulcrummHackReq - 10) { // Assume that if we're within 10, we'll get there by the time we've earned the invite
    ...
    for (const faction of factionWorkOrder) {
      ...
      if (preferredCompanyFactionOrder.includes(faction)) {
        // If we don't currently have the faction invite or are part of the faction, work for the invite.
        if (!allCurrentFactions.includes(faction))
...
/** @param {NS} ns */
async function getInvitedFactions(ns) {
  return await getNsDataThroughFile(ns, 'ns.checkFactionInvitations()');
}
...
async function earnFactionInvite(ns, factionName) {
  ...
  var invitations = await getInvitedFactions(ns);
...
export async function workForSingleFaction(ns, factionName, forceUnlockDonations = false, forceBestAug = false) {
  ...
  ns.print(`Faction "${factionName}" Highest Aug Req: ${highestRepAug.toLocaleString()}, Current Favor (${startingFavor}/150) Req: ${favorRepRequired.toLocaleString()}`);
  let lastStatusUpdateTime = Date.now() - statusUpdateInterval;
